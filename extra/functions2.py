...
def find_predictions(synthesis, filter, z, UVP_max, find_fDNS=True):
    ...
    if (NUM_CHANNELS==1):
        ...
        _, U_DNS = synthesis([z[0], z[1], LES_U], training=False)
        _, V_DNS = synthesis([z[0], z[1], LES_V], training=False)
...
@tf.function
def find_scaling(UVP_sub, UVP_max, gfilter, fnUVPo=None, nfUVPo=None, fUVP_amaxo=None, nUVP_amaxo=None, subSection=True, findNewValues=True):
        ...
        if (subSection):        
            fU = gfilter(U[NY2-1:NY2,:,N2L:N2R,N2L:N2R], training=False)
            fV = gfilter(V[NY2-1:NY2,:,N2L:N2R,N2L:N2R], training=False)
            fP = gfilter(P[NY2-1:NY2,:,N2L:N2R,N2L:N2R], training=False)
        else:
