...
def findLESTerms(pLES):
    ...
    global UVP_DNS    # remember: if values gets modified then need to be declared as global
    global fnUVPo, nfUVPo, fUVP_amaxo, nUVP_amaxo
    global pPrint, simtimeo, pStepo
    global dlatents, dlatents_z1, LES_in0
    ...
    with nvtx.annotate("prepare", color="yellow"): 
        ...
        bfU = tf.convert_to_tensor(fU, dtype=DTYPE)
        bfV = tf.convert_to_tensor(fV, dtype=DTYPE)
        bfP = tf.convert_to_tensor(fP, dtype=DTYPE)
        bfU = tf.transpose(bfU, [1,0,2])
        bfV = tf.transpose(bfV, [1,0,2])
        bfP = tf.transpose(bfP, [1,0,2])
        bfU = bfU[:,tf.newaxis,:,:]
        bfV = bfV[:,tf.newaxis,:,:]
        bfP = bfP[:,tf.newaxis,:,:]
        bUVP = tf.concat([bfU,bfV,bfP], axis=1)
        ...
        if (NUM_CHANNELS==1):
            zAll = [dlatents, dlatents_z1, LES_all, nfimgA]
        else:
            LES_all = []
            for res in range(2,RES_LOG2-FIL+1):
                rs = 2**(RES_LOG2-FIL-res)
    ...
    with nvtx.annotate("prediction", color="purple"):
        ...
        UVP_DNS, UVP_LES, fUVP_DNS = wl_find_predictions(wl_synthesis, gfilter, zAll, UVP_max)
        resREC, resLES, resDNS, loss_fil = find_residuals(UVP_DNS, bUVP, fUVP_DNS, UVP_DNS, bUVP, typeRes=0)
        ...
        # tune via latent space interpolation
        if (TUNE and (pStep%freqTollDNS==0 and pStep>0)):
            resDNStot.append(resDNS.numpy())
            it  = 0
            while (resREC>tollDNS and it<lr_DNS_maxIt):
                ...
                UVP_DNS, UVP_LES, fUVP_DNS, resREC, resLES, resDNS, loss_fil = \
                    step_find_dlatents_mDNS(wl_synthesis, gfilter, opt_kDNS, zAll, UVP_DNS, bUVP, ltv_DNS, UVP_max, typeRes=0)
                ...
                if (it%1==0 and it>0):
                    print("LES iterations:: it {0:6d} simtime {1:3e} resREC {2:3e} resLES {3:3e} resDNS {4:3e} loss_fil {5:3e}" \
                        .format(it, simtime, resREC.numpy(), resLES.numpy(), resDNS.numpy(), loss_fil))

                # move next iteration
                it = it+1

            print("LES final iterations:: it {0:6d} simtime {1:3e} resREC {2:3e} resLES {3:3e} resDNS {4:3e} loss_fil {5:3e}" \
                .format(it, simtime, resREC.numpy(), resLES.numpy(), resDNS.numpy(), loss_fil))
    ...
    if (simtime>=pPrint):
        ...
        filename = "./results_StylES/resDNStot.txt"
        np.savetxt(filename, np.c_[resDNStot], fmt='%1.4e')
