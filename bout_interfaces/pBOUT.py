...
managerCheckpoint = tf.train.CheckpointManager(checkpoint,
    '/lustre/scafellpike/local/HT04543/jxc06/jxc74-jxc06/projects/Turbulence_with_Style/PhaseIII_FARSCAPE3/codes/StylES/checkpoints/',
    max_to_keep=1)
...
def initFlow(npv):
    ...
    if (LOAD_DATA):
        ...
        resREC, resLES, resDNS, UVP_DNS, UVP_LES, fUVP_DNS, loss_fil = step_find_latents_LES_restart_A(wl_synthesis, filters[IFIL], w0, w1)
        ...
        fU_DNS = filters[IFIL](U_DNS, training = False)
        fV_DNS = filters[IFIL](V_DNS, training = False)
        fP_DNS = filters[IFIL](P_DNS, training = False)
...
def find_bracket(F, G, spacingFactor):
    ...
    # filter
    fpPhi_DNS = filters[IFIL](pPhi_DNS[tf.newaxis,tf.newaxis,:,:], training=False)
...
def findLESTerms(pLES):
    ...
    it = 0
    tstart = time.time()
    resREC, resLES, resDNS, UVP_DNS, UVP_LES, fUVP_DNS, loss_fil = step_find_residuals(wl_synthesis, filters[IFIL], w0, w1, fimgA, INIT_SCAL)
    # print("Starting residuals:  resREC {0:3e} resLES {1:3e}  resDNS {2:3e} loss_fill {3:3e} " \
    #     .format(resREC.numpy(), resLES.numpy(), resDNS.numpy(), loss_fil))
    ...
    while (resREC.numpy()>tollLES and it<maxit):
        ...
        resREC, resLES, resDNS, UVP_DNS, UVP_LES, fUVP_DNS, loss_fil = \
            step_find_latents_LES(wl_synthesis, filters[IFIL], opt_LES, w0, w1, fimgA, ltv_LES, INIT_SCAL)
        ...
        if (it!=0 and (it%100==0)):
            tend = time.time()
            print("\nLES iterations:  time {0:3e}   step {1:6d}   it {2:6d}  residuals {3:3e} resLES {4:3e}  resDNS {5:3e} loss_fill {6:3e}  lr {7:3e} " \
